/**
 * SAS similar triangles (research candidate, LENGTH layer).
 *
 ...
 *
 *   eqratio(A,B,D,E, C,A,F,D)   — AB/DE = CA/FD   (remaining proportion)
 *   eqangle(B,A,C, E,D,F)       — ∠BAC = ∠EDF
 *   eqangle(B,C,A, E,F,D)       — ∠BCA = ∠EFD
 *
 * HOW IT PLUGS INTO THE HARNESS
 * The research verifier (`lengths/verify.ts`) filters `eqratio` premises OUT of
 * the ordinary-fact list handed to `derive`, then runs `LengthAR` over
 * (cited facts ∪ one-step DD/length consequences). So this rule cannot see the
 * cited `eqratio(A,B,D,E,B,C,E,F)` directly; instead it fires off the cited
 * `eqangle`, identifies the correspondence numerically, and emits the BRIDGE
 * proportion
 *
 ...
 *   two arms, then GUARD numerically: the included angle equality AND the
 *   two-sides proportion AB/DE = BC/EF must both hold in the coordinates before
 *   anything is emitted. A figure with equal angle but non-proportional sides
 *   (or proportional sides but a different included angle) is rejected.
 * - Both arm-correspondences are tried; only the one whose included-side
 *   proportion actually holds survives, so a generic (scalene) figure pins the
 *   correspondence uniquely.
 * - Similarity allows MIRROR images, so orientation is never constrained.
 * - Every emitted fact is numerically gated (`factHoldsL` / `factHolds` via
 *   `factHoldsL`), so a false fact is never produced.
 */
...
import { canonicalKeyL, eqratio, factHoldsL, type LFact, type LRule } from "../dsl";
  ...
  derive(cited, { coords }) {
    ...
    for (const eq of eqanglesOf(cited)) {
      ...
      for (const [d2, f2] of armMatchings) {
        ...
        // GUARD 2 — the two pairs of INCLUDED sides must be in proportion:
        //   |ba|/|e d2| = |bc|/|e f2|   (i.e. AB/DE = BC/EF for this matching).
