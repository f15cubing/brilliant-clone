...
describe("tangent-secant power (research length rule)", () => {
  ...
  it("the equivalent pairing MA/MR = MB/MA verifies too (same fact up to symmetry)", () => {
    const r = verifyL([tangent_secant_power], {
      coords,
      bindings: {},
  ...
  it("GAP: neither the shipped engine nor the existing length rules can derive it", () => {
    // eqratio is not in the shipped DSL and AR is angles-only; the existing
    // length rules (similar/Thales/SAS-sim/chord-secant power) need a 4th cyclic
    // point or cited equal-angles, which a tangent-only config does not provide.
