 ...
 * the two SAS givens (AB/DE = BC/EF and the included ∠ABC = ∠DEF) and every
 * derived fact hold exactly. s≠1 ⇒ non-congruent ⇒ a genuine SIMILARITY.
 */
const A: V = [0, 0];
const B: V = [6, 0];
const C: V = [1, 4];
...
describe("SAS similar triangles (research length rule)", () => {
  ...
  it("the rule emits the bridge proportion and the two remaining equal angles", () => {
    ...
    const has = (g: Fact) =>
      out.some((f) => f.kind === "rel" && factHoldsL(f, coords) && JSON.stringify(f) === JSON.stringify(g));
  ...
  it("verifies the derived proportion AB/DE = CA/FD citing the ratio + included-angle premises", () => {
    const r = verifyL([sas_similarity], {
      coords,
      bindings: {},
  ...
  it("MINIMALITY: dropping either SAS premise → not valid", () => {
    ...
    const dropRatio = verifyL([sas_similarity], {
      coords,
      bindings: {},
      establishedFacts: [ratioPrem, angB],
      candidateFact: goal,
    ...
    const dropAngle = verifyL([sas_similarity], {
      coords,
      bindings: {},
      establishedFacts: [ratioPrem, angB],
      candidateFact: goal,
  ...
  it("GAP: the shipped engine (RULES) cannot validate the side proportion", () => {
    // `eqratio` is not in the shipped DSL and AR is angles-only, so the shipped
    // verifier cannot establish this ratio at all.
    const r = verifyWith(RULES, {
      coords,
      bindings: {},
      establishedFacts: [angB],
