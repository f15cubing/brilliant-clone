/**
 * Power of a point (research candidate, LENGTH layer).
 *
 ...
 *
 * and additionally the equivalent pairing PA/PD = PC/PB. These are ratio facts
 * the shipped engine cannot otherwise produce (AR is angles-only and `eqratio`
 * is not in the shipped DSL — see the test's GAP check). The companion problem
 * `problems/power_of_a_point.ts` proves the SAME fact the long way
 * (inscribed-angle ⇒ AA-similarity ⇒ length-chase); this rule collapses that to a
 * SINGLE deduction directly from the concyclicity + the two collinearities.
 *
 * PREMISES (ordinary facts; the length verifier filters `eqratio` premises out of
 * the fact list handed to `derive`, but these are not ratios):
