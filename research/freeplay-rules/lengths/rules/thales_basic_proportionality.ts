/**
 * Basic proportionality theorem / Thales' intercept theorem (research candidate,
 * LENGTH layer).
 *
 ...
 *
 * These are RATIO facts the engine cannot otherwise produce: the angle AR has no
 * length table, no shipped DD rule yields a ratio, and `eqratio` is not even in
 * the shipped DSL — so this is a genuine GAP (see the test's gap check).
 *
 * SOUNDNESS
 ...
 *   reject anything but a between-the-vertices cut.
