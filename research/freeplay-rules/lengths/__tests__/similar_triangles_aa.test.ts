...
import { similar_triangles_aa } from "../rules/similar_triangles_aa";
 ...
 * the two cited equal angles and all three side proportions hold exactly (we
 * check them with `factHoldsL`). s≠1 makes the triangles non-congruent, so this
 * is a genuine SIMILARITY (not a rigid motion).
 */
const A: V = [0, 0];
const B: V = [5, 0];
const C: V = [2, 3];
...
describe("AA similar triangles (research length rule)", () => {
  ...
  it("verifies the eqratio citing exactly the two eqangle facts", () => {
    const r = verifyL([similar_triangles_aa], {
      coords,
      bindings: {},
  ...
  it("MINIMALITY: dropping either equal-angle premise → not valid", () => {
    const dropA = verifyL([similar_triangles_aa], {
      coords,
      bindings: {},
      establishedFacts: [angA, angB],
      candidateFact: goal,
    ...
    const dropB = verifyL([similar_triangles_aa], {
      coords,
      bindings: {},
      establishedFacts: [angA, angB],
      candidateFact: goal,
