...
- Dynamic / draggable figures. v1 renders a **fixed** figure. _Dynamic rendering returns in P2._
...
- **Forward, single-step, rule-based checker** (`verify.ts`): a step is accepted iff (1) the asserted fact is numerically true in the figure **and** (2) some rule in the library derives it in one step from exactly the cited facts. Reasons returned: `not_true`, `unknown_premise`, `unjustified`. This matches the PRD's anti-cheese intent (true-but-uncited does not advance).
