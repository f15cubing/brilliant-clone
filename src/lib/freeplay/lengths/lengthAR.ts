/**
 * Algebraic Reasoning for LENGTHS — the metric companion to `../ar.ts`. Where
 * the angle AR keeps a Gaussian-elimination table over directed line angles
 * (mod 180°), this keeps the SAME kind of exact-rational table but over
 * LOG-LENGTHS: one generator per UNORDERED segment {P,Q} standing for log|PQ|.
 * Multiplicative facts become linear equations over logs:
 *
 ...
 * contract of AlphaGeometry's `ar.py` `Table`.
 *
 ...
 *
 * SCOPE LIMIT (intentional): `midp` is encoded only as logMA − logMB = 0 (the two
